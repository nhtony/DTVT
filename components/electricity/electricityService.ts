...
@Service()
class ElectricityService extends CRUD {
    ...
    async updateElectric(obj: any, where: any) {
        this.createQueryBuilder(NAME);
        this.update(obj);
        this.where(where);
        const sql = this.getQuery();
    ...
    async delete(subjectId: string) {
        return await this.pool.query(`DELETE FROM MON_HOC_DIEN WHERE MA_MON_HOC = '${subjectId}'`);
    }
    ...
    async count(){
        return await this.pool.query('SELECT COUNT(*) AS total FROM MON_HOC_DIEN;');
    }
}
